...
fun AddHabitDialog(
    onDismiss: () -> Unit,
    onConfirm: (String, TaskTriggerType, Int?) -> Unit,
    modifier: Modifier = Modifier
) {
    var title by remember { mutableStateOf("") }
    var selectedTriggerType by remember { mutableStateOf(TaskTriggerType.DAILY) }
    var thresholdMinutes by remember { mutableStateOf(90f) }
    var thresholdTss by remember { mutableStateOf(100f) }
                    ...
                    if (selectedTriggerType == TaskTriggerType.TRIGGER_LONG_DURATION) {
                        Column(
                            ...
                            Slider(
                                ...
                                valueRange = 30f..300f,
                                steps = 26, // 10-minute steps: (300-30)/10 = 27 steps
                                modifier = Modifier.fillMaxWidth()
                            )
                        }
                    }
                    ...
                    if (selectedTriggerType == TaskTriggerType.TRIGGER_HIGH_TSS) {
                        Column(
                            ...
                            Slider(
                                ...
                                valueRange = 50f..200f,
                                steps = 14, // 10 TSS steps: (200-50)/10 = 15 steps
                                modifier = Modifier.fillMaxWidth()
                            )
                        }
                    }
        ...
        confirmButton = {
            Button(
                onClick = {
                    ...
                    onConfirm(title, selectedTriggerType, threshold)
                },
